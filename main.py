...
class EnterpriseBot:
    ...
    def manage_running_trades(self):
        ...
        try:
            for pos in positions:
                ...
                # 3. Echte Strategie-Funktion aufrufen (check_entry_signal statt analyze!)
                    current_signal, _ = self.adv_engine.check_entry_signal(symbol, df_m5_live, self.vp_engine)
        
                    # 4. Live-Wahrscheinlichkeit von der KI holen
                    ai_live = self.ai.get_ai_prediction(symbol, df_m5_live, tf_name="M5")
                    if current_signal == "LONG":
                        current_prob = ai_live['long']
                    elif current_signal == "SHORT":
                        current_prob = ai_live['short']

                # 5. Reverse Check aufrufen
                self.check_stop_and_reverse(pos, current_price, symbol, current_signal, current_prob)

                # Reverse Check (Jetzt MIT dem Live-Signal übergeben)
                if self.check_stop_and_reverse(pos, current_price, symbol, current_signal, current_prob):
                    continue
    ...
    def check_stop_and_reverse(self, pos, current_price, symbol, current_signal, ai_prob):
        """
        ...
        if pos.type == self.mt5.mt5.ORDER_TYPE_BUY and current_signal == "SHORT" and ai_prob >= 60.0:
            should_reverse = True
            new_side = "SHORT"
            
        # Short Trade, aber das aktuelle Signal schreit plötzlich LONG
        elif pos.type == self.mt5.mt5.ORDER_TYPE_SELL and current_signal == "LONG" and ai_prob >= 60.0:
            should_reverse = True
            new_side = "LONG"
                ...
                for symbol in cfg.SYMBOLS:
                    try:
                        ...
                        if not tick or tick.ask == 0: 
                            #print(f"❌ {symbol}: is not activated or no tick data.")
                            continue
                        ...
                        THRESHOLD_M5, THRESHOLD_M1 = 0.65, 0.65 
